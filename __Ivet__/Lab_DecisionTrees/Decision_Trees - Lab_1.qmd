...
- Mirar presència de NAs. El professor ha comentat que els NAs no afecten als arbres: l'arbre es construiria sense aquella variable. No em queda molt clar: fer recerca del tema.
- Mirar presència d'Outliers.
- Identificar distribucions de variables asimètriques i aplicar-li's alguna transformació si es veiés convenient.
- No cal fer re-escalament en els arbres.
</div>
...
<div style='color:LightSeaGreen'>En l'exemple: per seguir partint caldria tenir mínim 50 observacions i que a cada fill n'hi vagin a parar mínim 20.</div>
