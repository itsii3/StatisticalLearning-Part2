...
We now  use the `tree()` function from the `tree` package to fit a classification tree in order to predict `High` using all variables but `Sales`.
...
```{r chunk6}
plot(tree.carseats)
text(tree.carseats, pretty = 1, cex=0.8)
```
...
In order to properly evaluate the performance of a classification tree on these data, we must estimate the test error rather than simply 
...
The tree with 9 terminal nodes results in only 74 cross-validation errors.  We plot the  error rate as a function of both `size` and `k`.
