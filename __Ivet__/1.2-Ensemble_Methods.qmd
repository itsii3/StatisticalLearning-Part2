...
Models that perform only slightly better than random guessing are called __*weak learners*__.
Weak learners low predictive accuracy may be due to the predictor having high bias or high variance.

<center>
![](Assets/1.2-EnsembleMethods_insertimage_1.png)
</center>

**Note.** Trees may be *weak* learners:
...
In many situations  trees may be a good option (e.g. for simplicity and interpretability). But there are issues that, if solved, may improve performance.
...
When we try to improve weak learners we need to deal with the bias-variance trade-off. ([The bias-variance trade-off cheatsheet](https://sites.google.com/view/datascience-cheat-sheets#h.pc6lrwop0hai))

![The bias-variance trade-off.](Assets/1.2-Ensemble_Methods-Slides_insertimage_1.png){width=70%}

*How can a model be made less variable or less biased without this increasing its bias or variance?* There are distinct appraches to deal with this problem
...
Ensemble learning takes a distinct based on "*the wisdom of crowds*". <span style='color:LightSeaGreen'><em><b>Wisdom of crowds</b> is the idea that large groups of people are collectively smarter than individual experts when it comes to problem-solving, decision-making, innovating, and predicting. The idea is that the viewpoint of an individual can inherently be biased, whereas taking the average knowledge of a crowd can result in eliminating the bias or noise to produce a clearer and more coherent result.</em></span>
...
- **Bagging.** *Bagging*, *Random forests* and *Random patches* yield a smaller variance than simple trees by building multiple trees based on aggregating trees built on a subset of
  - individual (1), 
  - features (2) or 
  - both (3).
- **Boosting.** *Boosting* or *Stacking* combine distinct predictors to yield a model with an increasingly smaller error, and so reduce the bias. They differ on if do the combination
  - sequentially (1) or 
  - using a meta-model (2).
- **Hybrid learners.** *Hybrid techniques* combine approaches in order to deal with both variance and bias. The approach should be clear from their name:
...
Given $X\sim F()$, s.t. $\operatorname{Var}_F(X)=\sigma^2$, and given a s.r.s. $X_1, ..., X_n$ from $F$; if $\overline{X}=\frac{1}{N}\sum_{i=1}^n X_i$, then $\operatorname{Var}_F(\overline{X})=\frac{\sigma^2}{n}$.</p>
...
__*What means averaging trees?*__ Two questions arise here:

1. *How to go from $X$ to $X_1, ..., X_n$?* This will be done using *bootstrap resampling*.
2. *What means "averaging" in this context?* Depending on the type of tree:
    - Average predictions for regression trees.
    - Majority voting for classification trees.

## The bootstrap

*Bootstrap* methods were introduced to estimate the standard error of a statistic.
...
Let $X$ be a random variable with distribution function $F$, let $\mathbf{X}=X_1,\ldots,X_n$ be an i.i.d random sample of $F$ and, let $x_1,\dots, x_n$ be a realization of $\mathbf{X}$.
...
Given a sample,  $\mathbf{X}$ from a certain distribution, $X$, consider it a discrete random variable $X_e$ that sets mass $1/n$ to each of the observed $n$ points $x_i$:

![](Assets/1.2-Ensemble_Methods-Slides_insertimage_3.png){width=70%}
...
A *natural* way to estimate $\theta(F)$ may be to rely on __*plug-in estimators*__,  where $F$ in $\hat \theta (F)$ is substituted by some approximation (estimate) to $F$.
...
$$
\hat{\sigma}_{\overline{X}}=\frac{\hat{\sigma}(X)}{\sqrt{n}}=\frac{\sqrt{\frac{1}{n}\sum_{i=1}^n(x_i-\overline{x})^2}}{\sqrt{n}}=\sigma
_{\overline{X}}(F_n).
$$
...
presents the obvious drawback that, when the functional form $\sigma _{\hat{\theta}}(F)$ is unknown, it is not possible to carry out the substitution of $F$ by $F_n$.
...
$$
__*without having to to know the form of*__ $\sigma_{\hat\theta}(F)$.
...
The *bootstrap* allows to estimate the standard error from samples of  $F_n$, that is, substituting $F_n$ by $F$ carried out in the *sampling step*.
...
Here, $\sigma_{\hat \theta}^*$ is the bootstrap standard error of $\hat \theta$ and $\hat \sigma_{\hat \theta}^*$ the bootstrap estimate of the standard error of $\hat \theta$.
...
Samples ${\bf X^*}$, obtained through this procedure are called bootstrap samples or __*re-samples*__.

Resampling consists of *extracting samples of size $n$ of $F_n$*: 
${\bf X_b^{*}}$ is a random sample of size $n$ obtained *with replacement* from the original sample ${\bf X}$. Samples ${\bf X^*_1, X^*_2, ..., X^*_B }$, obtained through this procedure are called *bootstrap*\ samples or __*re-samples*__.
...
<center>
![](Assets/1.2-Ensemble_Methods-Slides_insertimage_2.png){width=60%}
</center>
...
<center>
![](Assets/fromRealWorld2BootstrapWorld.png)
</center>
...
The bootstrap was introduced as a way to provide standard error estimators. When used to compute $\hat f_{bag}(x)$ or $\hat G_{bag}(x)$, as described above, it provides direct estimators of a characteristic, not of their standard errors. However, the bagging process __*can also provide resampling based estimates of the precision of these estimators*__.
...
<center>
![](Assets/oobErrorEstimation.jpg)
</center>

These __*out-of-bag* (OOB) observations__ can be used to build an estimate of prediction error.
...
1. We start by splitting the dataset in test/train subsets
2. Next we build a bagged tree on the train subset and evaluate it on the test subset.
3. Last we try to interpret the results using "Variable importance"
...
Leo Breimann introduced a modification to bagging, he called __*Random forests*__, that aims at decorrelating trees as follows:
...
While growing a decision tree, during the bagging process, Random Forests perform *split-variable randomization*:
...
```{r, out.width = "100%"}
#| code-fold: true
knitr::include_graphics("Assets/RandomForests2.png")
```

__*How many variables per split?*__

The number of variables per split $m$ can be chosen using cross-validation, but the usual recommendation for random selection of variables at each split has been studied by simulation:
...
![Random Forests Algorithm, from chapter 11 in [@Boehmke2020]](Assets/RandomForestsAlgorithm1.png){width=90%}

![Random Forests Algorithm, from chapter 17 in [@Hastie2016]](Assets/RandomForestsAlgorithm.png){width=90%}
...
Moreover, among the more popular machine learning algorithms, RFs have the least variability in their prediction accuracy when tuning.
...
<div style='color:LightSeaGreen'>Amb poc esforç, ho fan molt bé.</div>
...
<center>
![](Assets/RandomForestsHyperparameters1.png){width=80%}
</center>
...
<center>
![](Assets/RandomForestsHyperparameters2.png){width=80%}
</center>
...
<center>
![](Assets/RandomForestsHyperparameters3.png){width=80%}
</center>

### Sampling scheme

- Default: Bootstrap sampling with replacement on 100% observations.
- Sampling size and replacement or not can affect diversity and reduce bias.
- Node size has strong influence on error and time.

<center>
![](Assets/RandomForestsHyperparameters4.png){width=80%}
</center>
...
Random forests have been thoroughly used in Bioinformatics [@Boulesteix2012].Bioinformatics data are often high dimensional with
...
![](Assets/baggingVsboosting1.png){width=49%}
![](Assets/baggingVsboosting2.png){width=49%}

Source: [Ensemble Learning: Bagging & Boosting](https://resources.experfy.com/ai-ml/ensemble-learning-bagging-boosting/)

So, *what is Boosting?* **Idea:** create a model that is better than any of its individual components by combining their strengths and compensating for their weaknesses.

For this, multiple weak models are *trained sequentially*, and each new model is trained to improve the errors made by the previous model. The final model is a weighted combination of all the models, and the weights are determined by the accuracy of each model. 
...
AdaBoost (Adaptive Boosting) directly reflects what has been described above as "boosting". Its main objective is to __*Improve classification accuracy by combining multiple "weak learners"*__.
...
The final prediction is a __*weighted combination*__ of all weak learners. AdaBoost implements a vector of weights to penalize those samples that were incorrectly inferred (by increasing the weight) and reward those that were correctly inferred (by decreasing the weight). 
...
AdaBoost was sucessfully used in face recognition (*task:* to identify whether an image contains a face or not).
...
Adaboost was a breakthrough algorithm that significantly improved the accuracy of ML models. However, Adaboost has its limitations.
...
It was developed to overcome the limitations of Adaboost. Takes a different approach that can be linked with Optimization by Gradient Descent.
...
- **XGBoost.** Optimized implementation that uses regularization to control overfitting and provide better accuracy.

**LightGBM.** Relies on a technique to reduce the number of samples used in each iteration. Faster training, good for large datasets.
