...
\begin{eqnarray*}
\theta &=& \mathbb{E}_F(X)=\theta (F) \\
\theta &=& \operatorname{Med}(X)=\left\{m:P_F(X\leq m)=\frac{1}{2}\right\}=
\theta (F).
\end{eqnarray*}

To estimate $\theta(F)$ we often rely on __*plug-in estimators*__: $\hat{\theta}=\theta (F_n)$

\begin{eqnarray*}
\hat{\theta}&=&\overline{X}=\int XdF_n(x)=\frac
1n\sum_{i=1}^nx_i=\theta (F_n)
\\
\hat{\theta}&=&\widehat{\operatorname{Med}}(X)=\left\{m:\frac{\# \left\{x_i\leq m\right\}}n=\frac{1}{2}\right\}=\theta
(F_n)
\end{eqnarray*}
...
The main idea is that the  *bootstrap* standard error of $\hat\theta$, $\sigma_B(\hat\theta)$ can be *approximated* by $\hat{\sigma}_B (\hat\theta)$.
...
knitr::include_graphics("Assets/ames2VIP.png")
```
:::
...
```{r, out.width="100%"}
knitr::include_graphics("Assets/RandomForests1.png")
```
...
knitr::include_graphics("Assets/RandomForestsAlgorithm.png")
```
